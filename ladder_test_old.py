...
activs_txt_file: str = "activs_part1.txt"
activs_bin_file: str = "activs_part1.bin"
...
with open(activs_txt_file, "r", encoding="utf-8")  as f:
    #
...
if not  os.path.exists(model_path_state_dict):
    #
    raise UserWarning(f"Error: model weights path not found : {model_path_state_dict}")
...
with open(activs_txt_file, "r") as futt: 
    ss = futt.readlines()
    for num_line, s in enumerate(ss):
        ...
        print(f"{num_line}/{len(ss)} | loss: ",loss.item())
...
#
flt: str = f"final loss : {final_loss}"
#
print(flt)
res += flt
...
res_file_name: str = "res_log_ladder_activations_testing.txt"
#
