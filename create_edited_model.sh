...
TENSORS_EXT="gld" N_TOK="$n_tok" build/bin/llama-cli -m $m -co -sp -p "$gld_prompt" -fa -ngl 80 -n 128 --no-warmup --temp 0
TENSORS_EXT="err" N_TOK="$n_tok" build/bin/llama-cli -m $m -co -sp -p "$err_prompt" -fa -ngl 80 -n 128 --no-warmup --temp 0
