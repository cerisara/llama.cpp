...
with open("MuSR/datasets/murder_mystery.json","r") as f:
    d = json.load(f)
    # print(d[0].keys())
    with open("qmurder_ok","w") as gok:
        with open("qmurder_ko","w") as gko:
            for i in range(len(d)):
                for j in range(len(d[i]['questions'])):
                    ...
                    s = "You are a detective who must solve the following mystery case: "
                    s += d[i]['context'] + z['question']+' '
                    s += 'Generate a short reasoning in less than 100 words to prove that the murderer is '+z['choices'][z['answer']]+':'
                    s=s.replace('\n',' ')
                    gok.write(s+'\n')
